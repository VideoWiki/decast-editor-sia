        ...
        <div class="flex flex-wrap justify-between items-center mb-4">
          <div class="flex items-center -mx-2 edit-subtitles m-2">
            ...
            <div class="px-2" style="transform: translate(0px, 3px)">
              <vs-tooltip text="Position Dropdown">
                <span></span>
                <position-dropdown
            ...
            <div class="px-2">
              ...
              >
                <div slot="trigger">
                  <vs-tooltip text="Font color">
                    <span></span>
                    <h4 class="text-primary">T</h4>
                  </vs-tooltip>
                </div>
              </v-swatches>
            </div>
            <div class="px-2">
              ...
              >
                <div
                  ...
                  style="width: 24px"
                >
                  <vs-tooltip text="Background color">
                    <span></span>
...
import AlignmentDropdown from './components/AlignmentDropdown.vue';
...
export default {
  ...
  methods: {
    ...
    scriptInputScene() {
      if (this.getElementById('scene_script_' + indexs).innerText === '') {
        this.$Progress.fail();
        this.$vs.loading.close();
        this.$vs.notify({
          title: 'ALERT:',
          text: 'Please add an Subtitle into Scene First',
          color: 'danger',
        });
        return true;
      }
    ...
    async prepareScene(indexs, taskId) {
      // if (this.count === 3 && taskId == null) {
      //   this.$vs.notify({
      //     title: 'ALERT:',
      //     text: 'Only 3 subtitle can be added to scenes in one go',
      //     color: 'warning',
      //   });
      //   return;
      // }
      ...
      this.$store
        ...
        .then((res) => {
          if (res.data.video_url) {
            ...
            this.getStatus(res.data.task_id, indexs);
          }
        })
        .catch((err) => {
    ...
    getStatus(taskId, indexs) {
      this.$store.dispatch('studio/getStatus', taskId).then((res) => {
        if (res.status === 'SUCCESS') {
          const response = {
            sceneNum: parseInt(indexs),
            value: res.output.video_url,
          };
  ...
  mounted() {
    ...
    localStorage.getItem('videoData', this.videoData);
